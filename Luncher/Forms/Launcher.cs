        ...
        private void Launch(string profileJson)
        {
            try
            {
                ...
                foreach (JProperty peep in jo["profiles"].Cast<JProperty>().Where(peep => peep.Name == Nickname.Text))
                {
                    ...
                    {
                        var resp = Request.DoPost(AuthShemes.Authserver + AuthShemes.Validate, new JObject { new JProperty("accessToken", jo["profiles"][peep.Name]["accessToken"]) }.ToString());
                        if (resp.Contains("Error"))
                        {
                            ...
                            var response = Request.DoPost(AuthShemes.Authserver + AuthShemes.Validate,
                                topost.ToString());
